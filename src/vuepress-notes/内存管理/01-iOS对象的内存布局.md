...
::: info 为什么
16 字节是 malloc 等分配器的最小分配单位，与对象实际需要多少无关。NSObject 只需 8 字节（isa），但系统会分配至少 16 字节。原因包括：内存对齐（64 位下 16 字节对齐效率更高）、分配器需要存储元数据、防止碎片、Apple 的 nano_malloc 等实现采用 16 字节为最小粒度。
:::
...
::: info 常见误解与正确理解
<strong>常见误解</strong>：认为所有继承自 NSObject 的对象都是 16 字节。

<strong>正确理解</strong>：只有 NSObject 本身是 8 字节（实例）/ 16 字节（分配）。有实例变量的对象大小会根据实例变量变化。
:::
