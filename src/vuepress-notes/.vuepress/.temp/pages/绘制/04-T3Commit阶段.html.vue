<template><div><h1 id="t3-catransaction-commit-阶段-提交到-render-server" tabindex="-1"><a class="header-anchor" href="#t3-catransaction-commit-阶段-提交到-render-server"><span>T3：CATransaction Commit 阶段（提交到 Render Server）</span></a></h1>
<table>
<thead>
<tr>
<th>项目</th>
...
<td>RunLoop 的 <code v-pre>kCFRunLoopBeforeWaiting</code> 阶段，或者手动调用 <code v-pre>CATransaction.commit()</code> 时</td>
</tr>
<tr>
...
<h2 id="三种-layer-树" tabindex="-1"><a class="header-anchor" href="#三种-layer-树"><span>三种 Layer 树</span></a></h2>
...
<td>动画进行时</td>
<td>应用进程内存中</td>
<td>存储动画的中间值，供应用层查询（如 <code v-pre>layer.presentation()</code>）</td>
</tr>
<tr>
...
<h2 id="动画和-runloop-的关系" tabindex="-1"><a class="header-anchor" href="#动画和-runloop-的关系"><span>动画和 RunLoop 的关系</span></a></h2>
...
<h2 id="presentation-tree-的真正作用" tabindex="-1"><a class="header-anchor" href="#presentation-tree-的真正作用"><span>Presentation Tree 的真正作用</span></a></h2>
<p><strong>核心结论</strong>：Render Server 才是计算中心</p>
<p>动画开始后，App 进程只提交一次，剩下的活儿全是 Render Server 的：</p>
<ol>
<li><strong>App 进程（t3 阶段）</strong>：把动画的&quot;剧本&quot;（起始值、终点值、时长、插值曲线函数）打包成一个 <code v-pre>CATransaction</code> 发送给 Render Server，只提交一次</li>
...
<td>它不是被主动&quot;计算&quot;出来提交的，而是当你访问 <code v-pre>layer.presentation()</code> 时，UIKit 才去向 Render Server 询问（或者根据时间戳本地推算）</td>
</tr>
<tr>
...
<h2 id="两种动画的本质区别" tabindex="-1"><a class="header-anchor" href="#两种动画的本质区别"><span>两种动画的本质区别</span></a></h2>
<table>
<thead>
<tr>
<th>对比项</th>
...
<td><code v-pre>UIView.animate</code>、<code v-pre>CABasicAnimation</code> 等</td>
<td>手动写了 <code v-pre>CADisplayLink</code>，并在回调里每帧修改 <code v-pre>view.center</code></td>
</tr>
</tbody>
</table>
</div></template>
