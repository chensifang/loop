...
function drawPaths(svg, container) {
    ...
    linkRows.forEach(row => {
        ...
        // 在容器内查找目标，避免匹配到页面标题等外部元素（如 VuePress 的 h1 id="sidetable"）
        const target = container.querySelector('#' + CSS.escape(linkTo)) || document.getElementById(linkTo);
        ...
        if (isHorizontal || isVertical) {
            ...
            // 使用三次贝塞尔曲线，控制点沿箭头方向延伸（与 git 历史一致，根据方向修正 control2）
            const offset1 = Math.max(30, deltaX * 0.3);
            const offset2 = Math.max(20, deltaX * 0.15);
            // 控制点1：沿箭头方向延伸（向目标方向弯曲，不向外）
            const control1X = startX < endX ? startX + offset1 : startX - offset1;
            const control1Y = startY;
            // 控制点2：从目标方向侧接近终点
            const control2X = startX < endX ? endX - offset2 : endX + offset2;
