...
::: info 关键理解
- <strong>锁是基于对象地址获取的</strong>：进入同步块时，锁已经基于 obj 的原始地址获取
- <strong>修改 obj 不影响已获取的锁</strong>：当前线程的锁不会受影响，会正常释放
- <strong>但会导致同步失效</strong>：其他线程使用 nil 同步时，会使用不同的锁，可能导致多个线程同时执行
- <strong>不会死锁或崩溃</strong>：运行时处理 nil 对象，不会导致死锁或崩溃
:::
